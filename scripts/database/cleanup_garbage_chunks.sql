...
-- Run against the glass_expert_ai database (documents_bgem3 table)
-- ============================================================
...
DELETE FROM documents_bgem3
WHERE id IN (
    SELECT id FROM documents_bgem3
    WHERE array_length(regexp_split_to_array(trim(content), '\s+'), 1) < 20
      AND source_type NOT IN ('qa_pair')  -- QA pairs can be short
);
-- Check: SELECT count(*) FROM documents_bgem3 WHERE array_length(regexp_split_to_array(trim(content), '\s+'), 1) < 20 AND source_type NOT IN ('qa_pair');
...
DELETE FROM documents_bgem3 a
USING documents_bgem3 b
WHERE a.id > b.id
  AND a.title = b.title
...
-- SELECT 'remaining' AS status, count(*) FROM documents_bgem3;
-- SELECT source_type, count(*) FROM documents_bgem3 GROUP BY source_type ORDER BY count(*) DESC;
-- SELECT language, count(*) FROM documents_bgem3 GROUP BY language;
